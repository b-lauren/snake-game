...
grid.style.width = `${width * 10 * 6}px`
grid.style.height = `${width * 10 * 6}px`

// create grid cells
for (let i = 0; i < width * width; i++) {
  const cell = document.createElement("div")
  cell.style.width = `${width * 6}px`
  cell.style.height = `${width * 6}px`
...
function startGame() {
  const audioElement = new Audio("sounds/game-start1.mp3");
  audioElement.play();
...
function gameLoop() {
  ...
  if (
    ...
    const audioElement = new Audio("sounds/violin-lose.mp3");
    audioElement.play();
    clearInterval(interval)
    return
  }
  ...
  if (cells[currentSnake[0]].classList.contains("food-item")) {
    ...
    score++
    const audioElement = new Audio("sounds/eating.mp3");
    audioElement.play();
    scoreDisplay.textContent = score
    createFood()
  }
  cells[currentSnake[0]].classList.add("snake")
  cells[currentSnake[0]].classList.add("eyes")
  cells[currentSnake[0]].innerText = "👀"
  snakeColor += snakeColorIncrement % 360
